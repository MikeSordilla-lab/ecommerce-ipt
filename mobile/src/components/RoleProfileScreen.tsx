...
export function RoleProfileScreen({ roleTitle, dashboardRoute, bottomNav }: RoleProfileScreenProps) {
  ...
  const hasPhotoSelected = !!image;

  return (
    <Screen bottomNav={bottomNav}>
      <Hero title={`${roleTitle} Profile`} subtitle="Manage your account." />
      ...
      <Card>
        <View style={styles.identity}>
          <Surface style={styles.avatar} elevation={0}>
            {avatarUri ? (
              ...
              <IconButton icon="account" size={32} iconColor={colors.primaryContainer} style={styles.noMargin} />
            )}
          </Surface>
        ...
        <View style={styles.uploadRow}>
          ...
          <Button title={uploading ? "Uploading..." : "Upload Photo"} icon="upload" disabled={!hasPhotoSelected || uploading} onPress={uploadImage} />
        </View>
      </Card>
      ...
      <Card>
        <Text variant="titleSmall" style={styles.sectionTitle}>Update Password</Text>
        <Text variant="bodySmall" style={styles.sectionHint}>Leave blank to keep current password.</Text>
